...
async function addExpense(e) {
    ...
    try {
        ...
        const response = await axios.post('http://localhost:2021/expense/postExpense', myExpense);
...
function createList(expense) {
    ...
    delBtn.onclick = async function (e) {
        ...
        try {
            if(id){
                const response = axios.delete(`http://localhost:2021/expense/delete-expense/${id}`)
...
document.addEventListener('DOMContentLoaded',async () => {
    ...
    try{
        const response = await axios.get('http://localhost:2021/expense/getExpense');
